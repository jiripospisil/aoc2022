    ...
    for _ in 0..MONKEYS_COUNT {
...

        {
            ...
            for piece in line.split(&[',', ' '][..]) {
                if let Ok(item) = i64::from_str(piece) {
                    items.push(item);
                }
            }
        }

        {
            ...
            if let Ok(val) = i64::from_str(pieces[2]) {
...

        {
            ...
            test = i64::from_str(pieces.last().unwrap()).unwrap();
        }
...
fn new_op(new_operator: &String, new_value: NewValue, item: i64) -> i64 {
...
fn process_round(monkey_idx: usize, modulo: i64, monkeys: &mut Vec<Monkey>) {
    let items = mem::take(&mut monkeys[monkey_idx].items);
    monkeys[monkey_idx].inspected += items.len() as u64;
    ...
    for item in items {
        ...
        monkeys[address].items.push(new_worry_level % modulo);
    }
}

fn main() {
    let mut monkeys = parse_monkeys();
    let modulo = monkeys.iter().fold(1i64, |acc, m| acc * m.test);

    for _ in 0..10_000 {
        for monkey_idx in 0..MONKEYS_COUNT {
            process_round(monkey_idx, modulo, &mut monkeys);
        }
    }
